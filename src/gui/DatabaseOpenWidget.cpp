...
void DatabaseOpenWidget::openDatabase()
{
    ...
    if (ok) {
        if (m_ui->quickUnlockCheckbox->isChecked() && !m_ui->editPassword->text().isEmpty()) {
            ...
            WindowsHello::storeKey(m_filename, keyData);
...
#ifdef WITH_XC_WINDOWSHELLO
        if (!WindowsHello::getKey(m_filename, passwordKey)) {
...
{
#if defined(WITH_XC_WINDOWSHELLO)
    return WindowsHello::hasKey(m_filename);
#elif defined(WITH_XC_TOUCHID)
...
#if defined(WITH_XC_WINDOWSHELLO)
        m_ui->quickUnlockCheckbox->setVisible(WindowsHello::isAvailable());
